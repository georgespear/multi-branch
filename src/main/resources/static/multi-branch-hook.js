(function($){
        ...
        $(document).on('click', '#add-job', function(e) {
            e.preventDefault();
            var existingJobs = $('.multi-branch').children('.job').length;
            var html = com.spear.bitbucket.multibranch.hook.addJob({
                'count' : existingJobs,
                'jobName' : '',
                'trigger' : '',
                'token' : '',
            ...
            $('.multi-branch').find('.delete-job').removeClass('hidden').addClass('inline-button');
        });
