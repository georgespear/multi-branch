...
public class PullRequestHook {
    ...
    public void triggerFromPR(PullRequest pullRequest, Trigger trigger) throws IOException {
        ...
        RepositoryHookSettings repositoryHookSettings = settingsService.getSettings(new RepositoryScope(repository));

        if (repositoryHookSettings == null) {
            return;
        }
        RepoSettings repoSettings = settingsService.getRepoSettings(repositoryHookSettings.getSettings());
